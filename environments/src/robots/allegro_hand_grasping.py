...
import environments.src.robots.allegro_hand_consts as ah_consts
...
def init_urdf_allegro():

    root_3d_models_robots = \
        Path(environments.__file__).resolve().parent / env_consts.GYM_ENVS_RELATIVE_PATH2ROBOTS_MODELS

    urdf = Path(root_3d_models_robots / ah_consts.ALLEGRO_HAND_GRIP_RELATIVE_PATH_URDF)
    return str(urdf)


class AllegroHand(RobotGrasping):

    def __init__(self, **kwargs):

        urdf = init_urdf_allegro()

        list_id_gripper_fingers = ah_consts.LIST_ID_GRIPPER_FINGERS
        list_id_gripper_fingers_actuated = ah_consts.LIST_ID_GRIPPER_FINGERS_ACTUATORS

        gripper_6dof_infos = ah_consts.GRIPPER_6DOF_INFOS
        gripper_parameters = ah_consts.GRIPPER_PARAMETERS

        gripper_default_joint_states = ah_consts.DEFAULT_JOINT_STATES

        max_standoff_gripper = ah_consts.MAX_HAND_STANDOFF_ALLEGRO
        wrist_palm_offset_gripper = ah_consts.WRIST_PALM_OFFSET
        half_palm_depth_offset_gripper = ah_consts.HALF_PALM_DEPTH

        pose_relative_to_contact_point_d_min = ah_consts.POSE_RELATIVE_TO_CONTACT_POINT_D_MIN
        pose_relative_to_contact_point_d_max = ah_consts.POSE_RELATIVE_TO_CONTACT_POINT_D_MAX
    ...
    def _set_robot_default_state(self):
        pdb.set_trace()
        for j_id, pos in ah_consts.DEFAULT_JOINT_STATES.items():
    ...
    def _cvt_genome2synergy_label(self, synergy_label, debug=False):
        ...
        synergy_label_projected = np.round(project_from_to_value(
            ...
            interval_to=ah_consts.FIXED_INTERVAL_SYNERGIES,
            x_start=synergy_label
        ))

        synergy_str = ah_consts.SYNERGIES_ID_TO_STR[synergy_label_projected]
        list_id_grip_fingers_actuated = ah_consts.SYNERGIES_STR_TO_J_ID_GRIP_FINGERS_ACTUATED[synergy_str]

        if debug:
            return list_id_grip_fingers_actuated, synergy_str
    ...
    @staticmethod
    def cvt_genome2synergy_label(synergy_label):
        ...
        return synergy_label_projected

    @staticmethod
    def get_synergies_id_to_str():
        return ah_consts.SYNERGIES_ID_TO_STR
