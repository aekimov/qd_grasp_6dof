...
import environments.src.robots.robotiq_2f_consts as r2f_consts
...
def init_urdf_robotiq_2f_85():

    root_3d_models_robots = \
        Path(environments.__file__).resolve().parent / env_consts.GYM_ENVS_RELATIVE_PATH2ROBOTS_MODELS

    urdf = Path(root_3d_models_robots / r2f_consts.ROBOTIQ_2_FINGERS_85_GRIP_RELATIVE_PATH_URDF)
    return str(urdf)


class Robotiq2Fingers(RobotGrasping):

    def __init__(self, **kwargs):
        urdf = init_urdf_robotiq_2f_85()

        list_id_gripper_fingers = r2f_consts.LIST_ID_GRIPPER_FINGERS
        list_id_gripper_fingers_actuated = r2f_consts.LIST_ID_GRIPPER_FINGERS_ACTUATORS

        gripper_6dof_infos = r2f_consts.GRIPPER_6DOF_INFOS
        gripper_parameters = r2f_consts.GRIPPER_PARAMETERS

        gripper_default_joint_states = r2f_consts.DEFAULT_JOINT_STATES

        max_standoff_gripper = r2f_consts.MAX_HAND_STANDOFF_ROBOTIQ
        wrist_palm_offset_gripper = r2f_consts.WRIST_PALM_OFFSET
        half_palm_depth_offset_gripper = r2f_consts.HALF_PALM_DEPTH

        pose_relative_to_contact_point_d_min = r2f_consts.POSE_RELATIVE_TO_CONTACT_POINT_D_MIN
        pose_relative_to_contact_point_d_max = r2f_consts.POSE_RELATIVE_TO_CONTACT_POINT_D_MAX
        super().__init__(
